...
| `ui_config.py` | Loads and validates `ui_config.json` — presentation-only UI settings (layout max width, color scheme); maps the scheme to NiceGUI's dark-mode value and the width to a CSS declaration, plus the navbar toggle's cycle/icon helpers; no NiceGUI dependency |
